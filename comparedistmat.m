function[spear,disterr,matdisterr]=comparedistmat(dtot,dsecond)
...
XYZA=GramtoXYZ(distmattoGram(dtot));
XYZB=GramtoXYZ(distmattoGram(dsecond));
[pts, VA]= convhulln(XYZA(:,:));
[pts, VB]= convhulln(XYZB(:,:));

polyg2=VA^(1/3)/VB^(1/3)
